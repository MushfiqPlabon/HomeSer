...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('Database query optimization completed')
        )
    ...
    def optimize_user_queries(self):
        ...
        self.stdout.write('    users = User.objects.select_related("clientprofile").all()')
        self.stdout.write('    for user in users:')
        self.stdout.write('        print(user.clientprofile.bio)')
    ...
    def optimize_service_queries(self):
        ...
        self.stdout.write('    services = Service.objects.only("name", "average_rating").order_by("-average_rating")')
        self.stdout.write('    for service in services:')
        self.stdout.write('        print(service.name, service.average_rating)')
    ...
    def optimize_cart_queries(self):
        ...
        self.stdout.write('        Prefetch("items", queryset=CartItem.objects.select_related("service"))')
    ...
    def optimize_order_queries(self):
        ...
        self.stdout.write('')
        self.stdout.write('    # After (single query with prefetching):')
        self.stdout.write('    orders = Order.objects.filter(user=request.user).prefetch_related(')
        self.stdout.write('        Prefetch("orderitem_set", queryset=OrderItem.objects.select_related("service"))')
        self.stdout.write('    )')
        self.stdout.write('    for order in orders:')
    ...
    def optimize_review_queries(self):
        ...
        self.stdout.write('    reviews = Review.objects.filter(service=service).select_related("user", "service")')
        self.stdout.write('    for review in reviews:')
        self.stdout.write('        print(review.user.username, review.rating)')