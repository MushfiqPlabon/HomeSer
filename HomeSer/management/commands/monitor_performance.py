...
class Command(BaseCommand):
    ...
    def monitor_database_performance(self):
        ...
        start_time = time.time()
        carts = Cart.objects.prefetch_related(
            'items__service'
        ).all()[:10]
    ...
    def analyze_model_relationships(self):
        ...
        self.stdout.write('    ✓ Many-to-Many relationship with Order through OrderItem')
        self.stdout.write('    ✓ One-to-Many relationship with Review')
        ...
        self.stdout.write('    ✓ Many-to-Many relationship with Service through CartItem')
        ...
        self.stdout.write('    ✓ Many-to-Many relationship with Service through OrderItem')
    ...
    def provide_recommendations(self):
        ...
        self.stdout.write('    ✓ Use prefetch_related() for many-to-many and reverse foreign key relationships')
        ...
        self.stdout.write('    ✓ Use exists() instead of count() when checking for existence')
        self.stdout.write('    ✓ Use iterator() for large querysets to reduce memory usage')
        
        # Cache recommendations
        self.stdout.write('  Cache Optimizations:')
        self.stdout.write('    ✓ Cache frequently accessed data like services and reviews')
        ...
        self.stdout.write('    ✓ Avoid N+1 query problems with proper use of select_related and prefetch_related')
        self.stdout.write('    ✓ Use annotations instead of extra queries in loops')
        self.stdout.write('    ✓ Use Q objects for complex queries')
        self.stdout.write('    ✓ Use raw SQL for complex queries that can\'t be optimized with ORM')
        
        # Model recommendations
        self.stdout.write('  Model Optimizations:')
        self.stdout.write('    ✓ Use database constraints instead of application-level validation when possible')
        self.stdout.write('    ✓ Use proper field types for better performance')
        self.stdout.write('    ✓ Use database-level defaults instead of application-level defaults')
        self.stdout.write('    ✓ Use database indexes on frequently queried fields')
        self.stdout.write('    ✓ Normalize data to reduce redundancy but denormalize when needed for performance')