...
from HomeSer.models import Service, ClientProfile, Cart, CartItem, Order, OrderItem, Review
import random
    ...
    def handle(self, *args, **options):
        ...
        for username in client_usernames:
            if not User.objects.filter(username=username).exists():
                client = User.objects.create_user(username, f'{username}@example.com', 'password')
                client.role = 'client'
                client.save()
                ...
                clients.append(client)
                self.stdout.write(self.style.SUCCESS(f'Client user {username} already exists'))

        # Create services
        service_names = [
            'Plumbing Repair', 'Electrical Work', 'House Cleaning', 
            'Lawn Mowing', 'Painting Services', 'Furniture Assembly',
            'Carpet Cleaning', 'Window Washing', 'HVAC Maintenance',
            'Pest Control'
        ]
        ...
        for client in clients:
            if not ClientProfile.objects.filter(user=client).exists():
                ClientProfile.objects.create(
                    ...
                    social_links={'facebook': 'https://facebook.com/example', 'twitter': 'https://twitter.com/example'}
                )
                self.stdout.write(self.style.SUCCESS(f'Profile for {client.username} created'))
        ...
        for service in services:
            for client in clients:
                ...
                if not Review.objects.filter(user=client, service=service).exists():
                    ...
                    self.stdout.write(self.style.SUCCESS(f'Review created for {service.name} by {client.username}'))
