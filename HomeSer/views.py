...
from .serializers import UserSerializer, ClientProfileSerializer, ServiceSerializer, CartSerializer, OrderSerializer, ReviewSerializer
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.role == 'admin':
            ...
            queryset = User.objects.select_related('clientprofile').filter(id=self.request.user.id)
...
class ClientProfileViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.role == 'admin':
            ...
            queryset = ClientProfile.objects.select_related('user').filter(user=self.request.user)
...
class CartViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.annotate(
            annotated_total_price=Sum(
                models.F('cartitem_set__service__price') * models.F('cartitem_set__quantity')
            )
        )
    ...
    def remove_service(self, request):
        ...
        try:
            ...
            return Response({'status': 'service not in cart'}, status=status.HTTP_400_BAD_REQUEST)
...
class ReviewViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not Order.objects.filter(
            ...
            raise serializers.ValidationError("You can only review services you have completed orders for.")
...
def profile(request):
    # Optimize with select_related
    profile, created = ClientProfile.objects.select_related('user').get_or_create(user=request.user)
